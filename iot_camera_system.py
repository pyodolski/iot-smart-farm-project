...
SENSOR_UPLOAD_URL = f"{SERVER_BASE_URL}/product/upload-sensor"
...
def receive_capture_command():
    ...
    try:
        ...
        greenhouse_id = data.get('greenhouse_id')  # greenhouse_id 추가
        ...
        if action == 'capture_and_upload':
            print(f"📸 촬영 명령 수신 - 그룹 ID: {group_id}, IoT ID: {iot_id}, Greenhouse ID: {greenhouse_id}")
            print(f"🔍 받은 전체 데이터: {data}")
            
            # 비동기적으로 촬영 및 업로드 실행
            def capture_and_upload_async():
                print(f"🔍 촬영 시작 - group_id: {group_id}, iot_id: {iot_id}, greenhouse_id: {greenhouse_id}")
                # 1. 이미지 촬영
                image_path = capture_image(group_id, iot_id)
                if image_path:
                    # 2. 서버로 업로드 및 분석
                    print(f"📤 이미지 업로드 - group_id: {group_id}, iot_id: {iot_id}")
                    success = upload_image(image_path, group_id, iot_id)
                    if success:
                        # 3. 센서 데이터 전송 (greenhouse_id 사용)
                        if greenhouse_id:
                            print(f"📊 센서 데이터 전송 - iot_id: {iot_id}, gh_id: {greenhouse_id}")
                            upload_sensor_data(iot_id, greenhouse_id)
