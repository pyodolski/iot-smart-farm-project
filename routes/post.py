    ...
    try:
        cursor.execute('''
            SELECT b.*, 
                   ...
                   EXISTS(SELECT 1 FROM likes WHERE board_id = b.id AND user_id = %s) as is_liked
            FROM board b
            WHERE b.id = %s
        ...
        # 댓글 조회 - cdate 기준으로 최신순 정렬 (PostgreSQL용 TO_CHAR 사용)
        cursor.execute('''
            SELECT c.*, 
                   c.commenter = %s as is_author,
                   TO_CHAR(c.cdate, 'YYYY-MM-DD HH24:MI:SS') as formatted_date
            FROM comments c
            WHERE c.board_id = %s
        ...
        if conn:
            ...
            conn.close()
        print(f"게시글 조회 오류: {e}")
        return jsonify({'message': '게시글 조회 실패', 'error': str(e)}), 500
...
def toggle_like(post_id):
    ...
    # 좋아요 수 조회 및 현재 좋아요 상태 확인
    cursor.execute('SELECT COUNT(*) as count FROM likes WHERE board_id = %s', (post_id,))
    like_count = cursor.fetchone()['count']
    
    cursor.execute(
        'SELECT EXISTS(SELECT 1 FROM likes WHERE board_id = %s AND user_id = %s) as is_liked',
        (post_id, session['user_id'])
    )
...
def create_comment(post_id):
    ...
    try:
        ...
        # 게시글 작성자 정보 조회
        cursor.execute("""
            SELECT b.user_id as author_id, b.title
            FROM board b
            WHERE b.id = %s
        """, (post_id,))
        post = cursor.fetchone()
