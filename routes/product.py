...
def upload_sensor():
    try:
        ...
        print(f"📥 센서 데이터 수신 - gh_id: {gh_id}, iot_id: {iot_id}, 온도: {temperature}°C, 습도: {humidity}%")
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if not greenhouse_exists:
                    print(f"⚠️ greenhouse id {gh_id}가 존재하지 않음. 센서 데이터 저장 건너뜀")
                    # 오류를 반환하지 않고 성공으로 처리 (IoT 클라이언트가 계속 동작하도록)
