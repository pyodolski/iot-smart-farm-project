...
if len(sys.argv) < 3:
    print("❌ 오류: gh_id와 group_id 인자가 필요합니다.")
    print("사용법: python3 pi_clientF3_fixed.py <greenhouse_id> <group_id>")
    sys.exit(1)

try:
    gh_id = int(sys.argv[1])
    group_id = int(sys.argv[2])
    print(f"✅ greenhouse_id: {gh_id}, group_id: {group_id} 수신 완료")
except ValueError:
    print("❌ 오류: gh_id와 group_id는 정수여야 합니다.")
    sys.exit(1)
...
try:
    ...
    current_gh_id = gh_id  # ✅ 명령줄 인자로 받은 greenhouse_id 사용
    current_group_id = group_id  # ✅ 명령줄 인자로 받은 group_id 사용
    
    print(f"📍 사용할 greenhouse_id: {current_gh_id}, group_id: {current_group_id}, iot_id: {current_iot_id}")
    ...
    for row_idx in range(1):
        for col_idx in range(1):
            ...
            # ✅ 수정: group_id를 올바르게 전달
            capture_and_upload(current_group_id, current_iot_id, f"capture_after_move_r{current_row}_c{current_col}")
